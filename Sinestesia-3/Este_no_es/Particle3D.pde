...
class Particle3D {
    ...
    void move(PApplet p, float gravity, float friction) {
        ...
        // Límites X
        float minX = -containerHalfSize; // <-- CAMBIO
        float maxX = containerHalfSize; // <-- CAMBIO
        ...
        // Límites Y
        float minY = -containerHalfSize; // <-- CAMBIO
        float maxY = containerHalfSize; // <-- CAMBIO
        ...
        // Límites Z
        float minZ = -containerHalfSize; // <-- CAMBIO
        float maxZ = containerHalfSize; // <-- CAMBIO
...
void display() { 
        p.pushMatrix(); 
        // CAMBIO CLAVE: Trasladamos la partícula a su posición
        // RELATIVA al centro del contenedor (containerCenter),
        // ya que el centro ya fue trasladado y rotado en el draw() principal.
        p.translate(this.x, this.y, this.z);

        // Rotación de la propia partícula (independiente del cubo)
        p.rotateX(this.rotX);
        p.rotateY(this.rotY);
        p.rotateZ(this.rotZ);
